import { Button, Card, Empty, Input, List, message, Modal } from "antd";
...
const Forum: React.FC = () => {
  ...
  const fetchIssues = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://bracu-study-portal.onrender.com/api/v1/issues");
      ...
      for (const issue of response.data) {
        const commentsResponse = await axios.get(
          `https://bracu-study-portal.onrender.com/api/v1/comments/${issue._id}`
        );
  ...
  const postComment = async (issueId: string) => {
    ...
    try {
      await axios.post(
        "https://bracu-study-portal.onrender.com/api/v1/comments",
        {
          issueId,
          userName: user.name,
          userId: user.email,
  ...
  const editComment = async (commentId: string, issueId: string) => {
    ...
    try {
      ...
      await axios.post("https://bracu-study-portal.onrender.com/api/v1/comments/edit", {
        commentId,
        newCommentText,
      });
  ...
  const deleteComment = async (commentId: string) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(
            `https://bracu-study-portal.onrender.com/api/v1/comments/delete/${commentId}`
          ); // Pass commentId in URL
  ...
  const resolveIssue = async (issueId: string) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.post("https://bracu-study-portal.onrender.com/api/v1/issues/resolve", {
            issueId,
            userName: user.name,
          });
      ...
      {issues.length > 0 ? (
        issues.map((issue) => (
          ...
          >
            ...
            {/* Resolve Button */ console.log(issue)}
            {!issue.resolved && user?.email === issue.createdBy.userId && (
              <Button
                type="danger"
                style={{ marginTop: "10px" }}
